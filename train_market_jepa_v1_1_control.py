...
def train_control(reference, variant, output, resume=None):
    ...
    if (state.get("training_protocol_version") != "fixed_sample_budget_v1" or
            state["global_step"] != config["training"]["max_optimizer_steps"]):
        raise ValueError("matched controls require a completed fixed_sample_budget_v1 endpoint")
    ...
    protocol = {
        ...
        "reference_sampling_population_sha256": state["sampling_population_sha256"],
        "reference_max_optimizer_steps": state["max_optimizer_steps"],
        "reference_effective_batch_size": state["effective_batch_size"],
    ...
    trainer.fit(interval_callback=lambda _: _save_interval_outputs(output, trainer.history))
    ...
    write_json(output / "final_summary.json", {
        ...
        "global_step": restored["global_step"], "samples_seen": restored["samples_seen"],
        "checkpoint_sha256": file_hash(endpoint), "RB_read": False,
    })
