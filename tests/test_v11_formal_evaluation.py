...
def test_control_trainer_checkpoint_resume(tmp_path, variant):
    ...
    trainer = ControlTrainer(model, config, ds, torch.device("cpu"), samples_per_epoch=2, data_manifest_sha256="fixture")
    trainer.fit()
    ...
    restored = ControlTrainer(control_model(config["model"], variant, debug=True), config, ds,
                               torch.device("cpu"), samples_per_epoch=2, data_manifest_sha256="fixture")
...
def test_comparison_requires_rb_and_never_invents_h16_gate():
    ...
    assert comparison_status("Full", positive, positive, positive, positive) == "V1_1_SCALING_SUPPORTED"
    ...
    assert comparison_status("LateFusion", negative_h16, positive, positive, positive).startswith("BLOCKED_")
