...
- Historical eligibility is a dataset-initialization contract, configured by `history_week.years`, `history_week.capacity`, and `history_week.require_full_history`. Under the formal profile, `require_full_history=true`: an episode is eligible only when its own commodity's first reliable Weekly timestamp is at least the configured calendar-year interval before `main_start`. This is calendar coverage, not a `token_count == 156` test. The dataset precomputes `eligible_contracts_by_commodity`; early episodes are removed before anchor creation and sampling, with no retry or short-history fallback.
...
- Eligibility audit artifacts are `history_week_eligibility.csv` and `history_week_eligibility.json`, containing per-commodity bounds/counts and per-contract decisions. Current `/data/jepa/v1_1_raw` evidence leaves SH with zero contracts after the three-calendar-year rule, so formal training is hard-blocked pending an explicit human decision; the requirement is not relaxed automatically.
