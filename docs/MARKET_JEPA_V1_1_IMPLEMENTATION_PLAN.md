...
- Minute, Daily, Current Weekly, and every H16/H64/H256 target remain within one `contract_uid`. All horizons must be valid, so an invalid H256 makes the anchor invalid. Historical weekly may span contracts but each visible segment resets IMC and starts with `contract_boundary=1`.
- Capacities are fixed at Minute 512, Daily 256, Current Weekly 64, Historical Weekly 156. Sequences are left padded; padding masks use `True=PAD`. IMC validity uses `True=available` per coordinate. Daily truncation retains the latest 256 and is counted.
...
- Minute origin is the first valid window close/OI and the preceding same-contract 20-bar Volume median. Daily and Current Weekly Price/OI origins are the first observable minute Open/OI at main-start, never the final main-start Daily/Weekly value. Historical Weekly resets at every visible contract segment. No missing baseline is filled from the future.
- For every anchor, Current Daily is closed lifecycle days plus one Daily bar dynamically aggregated from same-contract minutes `<= anchor`; Current Weekly is closed clipped lifecycle weeks plus one similarly causal partial week. Both contexts include `bar_is_partial`; Daily also includes elapsed minutes and Weekly elapsed trading days. Cached final 1d/1w rows are never used for an in-progress period and must pass end-of-period parity audits.
...
- Training sampling is deterministic `Commodity -> valid episode -> valid anchor`, uniformly at each level. Checkpoint selection is always `fixed_budget_final`; validation is diagnostic only and cannot create a best checkpoint.
...
- Development smoke uses realistic B=2/B=8 capacities on CUDA when available, followed by 100 optimizer steps using only real train-role data. No full 50-epoch training or RB evaluation is started.
